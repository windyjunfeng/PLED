...
def main(temp_img_path,temp_label_path,split_train_txt,split_val_txt,dAdt_file):
    ...
    val_segtrans = Compose([Transpose(indices=(3, 0, 1, 2)),])
    ...
    for epoch in range(total_epoch):
        ...
        with tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1}/{total_epoch}", unit='batch') as pbar:
            for batch_data in train_loader:
                ...
                dAdt_train = dAdt.permute(3,0,1,2).unsqueeze(0).repeat(inputs.shape[0], 1, 1, 1, 1)
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    # dAdt shape: (H, W, D, 3) -> (3, H, W, D) -> (batch, 3, H, W, D)
                    dAdt_val = dAdt.permute(3,0,1,2).unsqueeze(0).repeat(val_images.shape[0], 1, 1, 1, 1)  # 这里不能是batch size，因为有些测试集的图片数不能被batch size整除
                ...
                if metric < best_metric:
                    ...
                    torch.save(model.state_dict(), r"best_metric_model_efieldcal_vector_array_fusion_1_dual_channels.pth")
...
if __name__ == "__main__":
    ...
    split_train_txt = r'/data/disk_1/zhoujf/large_data/e-field_dataset/cohort_lab_health/train_l.txt'
    split_val_txt = r'/data/disk_1/zhoujf/large_data/e-field_dataset/cohort_lab_health/val_l.txt'
