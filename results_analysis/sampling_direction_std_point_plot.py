...
sub_foldername=r'sampling_direction_evaluation_point_plot'
...
for key in metric_representation.keys():
    for classname in classnames.keys():
        ...
        df_sorted_1_std=df_sorted_1.groupby(['Direction (°)','Methods'])[metric_representation[key]].std().reset_index()
        desired_order=['Origin','Baseline','PLED']
        df_sorted_1_std['Methods']=pd.Categorical(df_sorted_1_std['Methods'],categories=desired_order,ordered=True)
        df_sorted_final=df_sorted_1_std.sort_values(by='Methods')
        plt.figure()
        ax=sns.pointplot(data=df_sorted_final, x='Direction (°)', y=metric_representation[key], hue='Methods', markers=['s','^','*'],linestyles=['dotted','--','-'],estimator='mean',errorbar=None,linewidth=1.5, markersize=7.5, palette='viridis',dodge=True)  # dodge为True可以使得处于横坐标同一位置处的多类别绘制时略有偏移，避免互相覆盖
        ax.legend_.remove()
