...
for key in metric_representation.keys():
    for classname in classnames.keys():
        ...
        df_sorted_1=df.sort_values(by='Direction (°)', ascending=True).reset_index(drop=True)
        ...
        df_grouped = df_sorted_1.groupby(['Direction (°)','Methods'])[metric_representation[key]].mean().reset_index()
        
        # Use matplotlib lineplot instead of seaborn pointplot to avoid pandas indexing issues
        plt.figure()
        ax = plt.gca()
        
        # Get viridis colormap
        colors = plt.cm.viridis(np.linspace(0.25, 0.75, len(desired_order)))
        linestyles = ['dotted', '--', '-']
        
        # Plot each method separately
        for i, method in enumerate(desired_order):
            method_data = df_grouped[df_grouped['Methods'] == method].sort_values(by='Direction (°)')
            if len(method_data) > 0:
                ax.plot(method_data['Direction (°)'].values, 
                       method_data[metric_representation[key]].values, linestyle=linestyles[i], 
                       linewidth=1.5, color=colors[i], label=method)
        
        ax.set_xlabel('Direction (°)')
        ax.set_ylabel(metric_representation[key])
        
        # Set x-axis ticks - you can customize this list to specify which ticks to show
        # Option 1: Show all unique Direction values (automatic)
        # unique_directions = sorted(df_grouped['Direction (°)'].unique())
        # ax.set_xticks(unique_directions)
        # ax.set_xticklabels(unique_directions)
        
        # Option 2: Manually specify ticks (uncomment and modify as needed)
        custom_ticks = [-90,-60,-30,0,30,60,90]  # Example: specify your desired tick values
