...
metric_representation={'mae':'MAE (V/m)','mre':'MRE','mse':'MSE','f_norm':'F-Norm Loss','psnr':'PSNR (dB)','pearson_corr':'PCC','mae_loss_mean':'MAE of Mean','mae_loss_edge':'AAE (V/m)','mre_loss_edge':'ARE'}
...
# sub_foldername=r'box_plot_viridis_nolegend_pvalue\total'
sub_foldername=r'box_plot_viridis_nolegend_39_based_gt_step1_pvalue\total'
...
for key in metric_representation.keys():
    ...
    for classname in classnames.keys():
        ...
        pairwise_results = pg.pairwise_tests(dv=metric_representation[key],between='Methods', data=class_data, parametric=True)  # T检验，只比较两个组所以不需要correction，参数化检验，pairwise_results.columns可以查看结果字典中的key有哪些
