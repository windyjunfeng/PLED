...
methods_folders={'Origin':'35_efield_angleres_1','Baseline':'31_based_step1_efield_angleres_1','PLED':'39_based_step1_efield_angleres_1'}
...
sampling_info_root_path=r'/data/disk_1/zhoujf/large_data/e-field_dataset/cohort_lab_health'
output_root_path=r'/data/disk_2/zhoujunfeng/code/efield_calculation/outputs/evaluation/cohort_lab_health/results_analysis'
sub_foldername=r'efield_angleres_1_sampling_direction_evaluation_point_plot'
...
for test_subject in test_subjects:
    sampling_info_path=os.path.join(sampling_info_root_path,test_subject,'local_sampling_scalp_labels_new_1010_center_angleres_1/affine_matrices_local_labels.json')
...
for key in metric_representation.keys():
    for classname in classnames.keys():
        ...
        df_sorted_1['Methods']=pd.Categorical(df_sorted_1['Methods'],categories=desired_order,ordered=True)
        df_sorted_final=df_sorted_1.sort_values(by='Methods')
        
        # df_sorted_final=df_sorted_1.sort_values(by='Methods').reset_index(drop=True)
        # # Ensure numeric columns are properly typed to avoid pandas indexing issues
        # df_sorted_final['Direction (°)'] = df_sorted_final['Direction (°)'].astype(int)
        # df_sorted_final[metric_representation[key]] = pd.to_numeric(df_sorted_final[metric_representation[key]], errors='coerce')
        
        # # Convert to numpy arrays to avoid pandas indexing compatibility issues
        # x_data = df_sorted_final['Direction (°)'].values
        # y_data = df_sorted_final[metric_representation[key]].values
        # hue_data = df_sorted_final['Methods'].values
        
        plt.figure()
        ax=sns.pointplot(data=df_sorted_final, x='Direction (°)', y=metric_representation[key], hue='Methods', markers=['s','^','*'],linestyles=['dotted','--','-'],estimator='mean',errorbar=None,linewidth=1.5, markersize=8, palette='viridis',dodge=True)  # dodge为True可以使得处于横坐标同一位置处的多类别绘制时略有偏移，避免互相覆盖
        # ax=sns.pointplot(x=x_data, y=y_data, hue=hue_data, markers=['s','^','*'],linestyles=['dotted','--','-'],estimator='mean',errorbar=None,linewidth=1.5, markersize=8, palette='viridis',dodge=True)  # dodge为True可以使得处于横坐标同一位置处的多类别绘制时略有偏移，避免互相覆盖
        # ax = sns.pointplot(data=df_sorted_final, x='Direction (°)', y=metric_representation[key], hue='Methods',
        #                    markers=['s', '^', '*'], linestyles=['dotted', '--', '-'], estimator='mean', errorbar=('sd',1),
        #                    linewidth=1.5, markersize=7.5, palette='viridis', dodge=True)
        ax.set_xlabel('Direction (°)')
        ax.set_ylabel(metric_representation[key])
        ax.legend_.remove()
