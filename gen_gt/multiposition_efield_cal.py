...
from simnibs.simulation import fem_me
...
class TMSoptimize():
    ...
    def _prepare(self):
        ...
        # if (self.target is None or len(self.target) != 3) and len(self.target)!=1:
    ...
    def run(self, cpus=1, allow_multiple_runs=False, save_mat=True, return_n_max=1, custom_region=None, custom_weights=False, custom_weights_path='',mesh2nii_path=''):
        ...
        if custom_region is None:
            ...
            #     bar = self.mesh.elements_baricenters()[:]
            #     self.target=bar[self.target]
            #     self.centre = bar[self.centre]
            # target_region = self._get_target_region()
            target_region = mesh2nii_path
        elif custom_region == 'mesh_triangle':
            # target_region = self.mesh.elm.elm_number[self.mesh.elm.elm_type == 2]
            target_region = self.mesh.elm.elm_number[self.mesh.elm.tag1 == 1002]  # gray_matter_triangle
            if self.centre.shape[0]==1:
                bar = self.mesh.elements_baricenters()[:]
                self.centre = bar[self.centre]
        else:
            bar = self.mesh.elements_baricenters()[:]
            self.centre = bar[self.centre]
            target_region = custom_region
        # pos_matrices = self._get_coil_positions()
        ...
        if self.method.lower() == 'direct':
            self._direct_optimize(cond_field, target_region, self.pos_matrices,
                                          cpus, keep_hdf5=True, custom_weights=custom_weights,
    ...
    def _direct_optimize(self, cond_field, target_region, pos_matrices, cpus, keep_hdf5=False,custom_weights=False,custom_weights_path=''):
        """
        Parameters:
        -----------
        ...
        # import pdb
        # pdb.set_trace()
        didt_list = [self.didt for i in pos_matrices.values()]
        # fn_hdf5 = os.path.join(self.pathfem, self._name_hdf5())
        # if os.path.isfile(fn_hdf5):
        #     os.remove(fn_hdf5)
        # dataset = 'E_magn'
        # if custom_weights:
        #     volumes = np.genfromtxt(custom_weights_path)
        # else:
        ...
        #         if custom_weights:
        #             # return np.dot(np.linalg.norm(E[target_region - 1], axis=1),volumes)/volumes.shape[0]
        #             return np.dot(np.linalg.norm(E[target_region - 1], axis=1),volumes[target_region-1])/volumes[target_region-1].shape[0]  # gray_matter_triangle
        #         else:
        #             return np.average(
        ...
        #     direction=direction
        # )
        fem_me.tms_many_simulations(
            self.mesh, cond_field,
            self.fnamecoil,
            pos_matrices, didt_list, self.pathfem,
            fn_hdf5='', dataset=None,roi_path=target_region,field='E',
            post_pro=None,
