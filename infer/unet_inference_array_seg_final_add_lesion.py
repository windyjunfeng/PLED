...
def inference_seg(temp_img_path,temp_lesion_path, weight_path,output_path):
    ...
    # lesion_files = os.listdir(temp_lesion_path)
    ...
    with tqdm(total=len(test_imgs)) as pbar:
        with torch.no_grad():
            for i in range(len(test_imgs)):
                ...
                test_img_tensor=rescale_array(test_img_tensor, 0.0, 1.0, dtype=np.float32)  # 与训练和评价时的ScaleIntensity()变换保持一致
                ...
                lesion_file = os.path.join(temp_lesion_path, os.path.basename(test_img))
                if os.path.exists(lesion_file):
                # if os.path.basename(test_img) in lesion_files:
                    # lesion_file=os.path.join(temp_lesion_path,os.path.basename(test_img))
                    lesion=np.load(lesion_file)
                    test_output_array=test_output_array+lesion*10
                    test_output_array[test_output_array==10]=0  # 推理的分割结果为背景的地方，忽略病灶，这样与label的合并病灶规则一致
...
if __name__ == "__main__":
    ...
    for line in lines:
        ...
        temp_lesion_path = os.path.join(temp_lesion_rootpath, line.rstrip('\n'), 'local_lesion')
        output_path = os.path.join(output_rootpath, line.rstrip('\n'),'inference','local_seg_final')
        inference_seg(temp_img_path, temp_lesion_path, weight_path, output_path)
