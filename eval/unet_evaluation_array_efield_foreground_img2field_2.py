...
def main(temp_img_path,temp_seg_path, temp_label_path,split_test_txt,weight_path, output_dir):
    num_classes = 1
    # num_labels=10
    num_labels = 20
    batch_size=32
    ...
    for line in lines_2:
        ...
        # temp_label_path_whole=os.path.join(temp_label_path, line.rstrip('\n'), 'local_efield')
        temp_seg_path_whole = os.path.join(temp_seg_path, line.rstrip('\n'), 'local_sampling_scalp_labels_new/local_label')
        ...
        for file in files:
            ...
            test_segs.append(os.path.join(temp_seg_path_whole,file))
        ...
        with torch.no_grad():
            ...
            for val_data in val_loader:
                ...
                for i in range(val_images.shape[0]):  # 这里不应该是batch size，因为最后一个batch不一定是满batch的
                    val_seg = torch.tensor(np.load(test_segs[index]))[None,None,:].to(device) # 由于dataloader是顺序依次加载的，因此可以根据顺序得到对应val_seg
                    edge_mask_batch = edge_mask(val_seg, num_labels, device).to(device)
                    # val_outputs_foreground=val_outputs[i][val_seg[0] != 0]
                    # val_labels_foreground=val_labels[i][val_seg[0] != 0]
                    # mae_loss_ = torch.mean(torch.abs(val_outputs_foreground - val_labels_foreground))
                    # mae_loss_edge_ = torch.mean(torch.abs((val_outputs_foreground - val_labels_foreground))[edge_mask_batch[0][val_seg[0] != 0]])
                    ...
                    # mre_loss_edge_ = torch.mean(torch.abs((val_outputs_foreground - val_labels_foreground) / val_labels_foreground)[(val_labels_foreground != 0) & edge_mask_batch[0][val_seg[0] != 0]])
                    ...
                    evaluation_items=evaluation_per_category(evaluation_items, val_outputs, val_labels, val_seg, edge_mask_batch,i)
                    index=index+1
                pbar.update(1)  # 更新进度条
...
def evaluation_per_category(evaluation_items,val_outputs,val_labels,val_seg,edge_mask_batch,batch_i):
    for k in evaluation_items:
        # import pdb
        # pdb.set_trace()
        val_images_category_flag=(val_seg[0] == int(k))
        ...
        mae_loss_ = torch.mean(torch.abs(val_outputs_foreground - val_labels_foreground))
        mae_loss_edge_ = torch.mean(torch.abs((val_outputs_foreground - val_labels_foreground))[edge_mask_batch[0][val_images_category_flag]])
        mae_loss_mean_ = torch.abs(torch.mean(val_outputs_foreground) - torch.mean(val_labels_foreground))
        mre_loss_ = torch.sum(torch.nan_to_num(torch.abs((val_outputs_foreground - val_labels_foreground) / val_labels_foreground) * (val_labels_foreground != 0), nan=0.0)) / torch.count_nonzero(val_labels_foreground)
        mre_loss_edge_ = torch.mean(torch.abs((val_outputs_foreground - val_labels_foreground) / val_labels_foreground)[(val_labels_foreground != 0) & edge_mask_batch[0][val_images_category_flag]])
        mse_loss_ = torch.mean((val_outputs_foreground - val_labels_foreground) ** 2)
        f_norm_ = torch.sqrt(torch.sum((val_outputs_foreground - val_labels_foreground) ** 2))
        psnr_ = 20*torch.log10(torch.max(torch.amax(val_outputs_foreground),torch.amax(val_labels_foreground)))-10*torch.log10(mse_loss_)
        mean_val_labels = torch.mean(val_labels_foreground)
...
if __name__ == "__main__":
    ...
    # temp_seg_path = r'/root/autodl-tmp/tms_e-field_scalp_data/segmentation/label'
    ...
    # weight_path=r'/root/autodl-fs/code/deep_learning_efield/segmentation/unet/35_best_metric_model_img2efieldcal_array_true_1_interpolate.pth'
    # output_dir=r'/root/autodl-tmp/tms_e-field_scalp_data/efield_calculation/evaluation/35_s'
    ...
    temp_seg_path = r'/media/irc300/d37c3a3a-b751-4342-9504-f7ee31232551/windyjunfeng/data/deep_learning_e-field/TMS-EEG_lesion_cortex/m2m_files_add_lesion'
    ...
    weight_path=r'/media/irc300/d37c3a3a-b751-4342-9504-f7ee31232551/windyjunfeng/code/deep_learning_e-field/segmentation/unet/35_best_metric_model_img2efieldcal_array_true_1_interpolate.pth'
    output_dir=r'/media/irc300/d37c3a3a-b751-4342-9504-f7ee31232551/windyjunfeng/data/deep_learning_e-field/TMS-EEG_lesion_cortex/evaluation/efield_calculation/35_add_lesion'
    main(temp_img_path, temp_seg_path, temp_label_path, split_test_txt,weight_path,output_dir)
