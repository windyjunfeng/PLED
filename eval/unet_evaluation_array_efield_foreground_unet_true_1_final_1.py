...
def main(temp_img_path,temp_label_path,split_test_txt,weight_path, dAdt_file, output_dir):
    num_classes = 1
    num_labels=10
    # num_labels=20
    batch_size=32
    ...
    # pdb.set_trace()
    for line in lines_2:
        temp_img_path_whole=os.path.join(temp_img_path, line.rstrip('\n'), 'local_sampling_scalp_labels_new_1010_center_angleres_1/local_label')
        # temp_img_path_whole = os.path.join(temp_img_path, line.rstrip('\n'),'inference','local_seg_final')
        temp_label_path_whole=os.path.join(temp_label_path, line.rstrip('\n'), 'mesh2nii_index_new_correct_interpolate_angleres_1/efield_calculation','local_efield')
    ...
    # category_names = ['1','2','3','4','5','9','11','12','13','14','15','19']
        ...
        with torch.no_grad():
            ...
            for val_data in val_loader:
                ...
                for k in conductivity_map:
                    val_images = torch.where(val_images == int(k),
                                             torch.tensor(conductivity_map[k]).to(torch.float32).to(device), val_images)
                dAdt_val = dAdt.unsqueeze(0).repeat(val_images.shape[0], 1, 1, 1, 1)
                dAdt_mask = torch.where(val_images == 0, torch.tensor(0).to(torch.float32).to(device), dAdt_val)
...
if __name__ == "__main__":
    ...
    # temp_img_path = r'/root/autodl-tmp/tms_e-field_scalp_data/segmentation/inference/local_seg_small'
    ...
    # weight_path=r'/root/autodl-fs/code/deep_learning_efield/segmentation/unet/38_best_metric_model_efieldcal_array_fusion_1_new_interpolate.pth'
    weight_path=r'/data/disk_2/zhoujunfeng/code/efield_calculation/weights/39_best_metric_model_efieldcal_array_fusion_1_new_interpolate_all.pth'
    dAdt_file = r'/data/disk_2/zhoujunfeng/code/efield_calculation/dadt_norm_808032_0mm_fmm3d.npy'
    # output_dir=r'/root/autodl-tmp/tms_e-field_scalp_data/efield_calculation/evaluation/38_based_step1'
    # output_dir=r'/media/irc300/d37c3a3a-b751-4342-9504-f7ee31232551/windyjunfeng/data/deep_learning_e-field/TMS-EEG_lesion_cortex/evaluation/efield_calculation/39_based_step1_add_lesion'
    output_dir=r'/data/disk_2/zhoujunfeng/code/efield_calculation/outputs/evaluation/cohort_lab_health/39_based_gtlabel_efield_angleres_1'
    main(temp_img_path, temp_label_path, split_test_txt,weight_path,dAdt_file,output_dir)
