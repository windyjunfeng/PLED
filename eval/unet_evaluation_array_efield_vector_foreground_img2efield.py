...
def main(temp_img_path,temp_seg_path, temp_label_path,split_test_txt,weight_path, output_dir):
        ...
        with torch.no_grad():
            ...
            for val_data in val_loader:
                ...
                for i in range(val_images.shape[0]):  # 这里不应该是batch size，因为最后一个batch不一定是满batch的
                    ...
                    # 边缘掩码（针对前景区域）
                    edge_mask_foreground = edge_mask_batch[0][foreground_mask] 
                    # MAE: 使用矢量幅值的平均绝对误差
    ...
    pearson_corr_metric=torch.mean(torch.tensor(pearson_corr)[torch.isfinite(torch.tensor(pearson_corr))])
...
def evaluation_per_category(evaluation_items,val_outputs,val_labels,val_seg,edge_mask_batch,batch_i):
    for k in evaluation_items:
        # import pdb
        # pdb.set_trace()
        val_images_category_flag=(val_seg[0] == int(k))  # (H, W, D)
        ...
        # 边缘掩码（针对前景区域）
        edge_mask_category = edge_mask_batch[0][val_images_category_flag]  # (N_category_spatial,)
        # MAE: 使用矢量幅值的平均绝对误差
        ...
        if torch.any(edge_mask_category):
            ...
            mae_loss_edge_ = torch.tensor(float('nan')).to(val_outputs.device)
        # 均值差异（使用矢量幅值）
        ...
        if torch.any(edge_mask_category) and torch.any(val_labels_magnitude_category[edge_mask_category] != 0):
            ...
            mre_loss_edge_ = torch.tensor(float('nan')).to(val_outputs.device)
        # MSE: 使用矢量幅值的均方误差
...
if __name__ == "__main__":
    ...
    temp_seg_path = r'/data/disk_1/zhoujf/large_data/e-field_dataset/cohort_lab_health'
    ...
    output_dir=r'/data/disk_2/zhoujunfeng/code/efield_calculation/outputs/evaluation/cohort_lab_health/45_efield_vector'
