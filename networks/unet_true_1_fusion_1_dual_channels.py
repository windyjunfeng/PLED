...
class UNet_true_1_fusion_1_dual_channels(nn.Module):
    '''
    Dual input UNet with different input channels for each branch
    x1: image input (1 channel)
    x2: dAdt input (3 channels)
    '''
    def __init__(
        self,
        spatial_dims: int,
        in_channels_1: int,  # 第一个输入的通道数（图像）
        in_channels_2: int,  # 第二个输入的通道数（dAdt）
        out_channels: int,
        ...
        def _create_block(
            inc1: int, inc2: int, outc: int, channels: Sequence[int], strides: Sequence[int], is_top: bool
        ) -> nn.Module:
            """
            ...
            Args:
                inc1: number of input channels for branch 1.
                inc2: number of input channels for branch 2.
            ...
            if len(channels) > 1:
                subblock = _create_block(c, c, c, channels[1:], strides[1:], False)  # continue recursion down
                upc = c * 2
                bottom_flag = False
            else:
            ...
            down1 = self._get_down_layer(inc1, c, s, is_top)  # create layer in downsampling path for branch 1
            down2 = self._get_down_layer(inc2, c, s, is_top)  # create layer in downsampling path for branch 2
        ...
        self.model = _create_block(in_channels_1, in_channels_2, out_channels, self.channels, self.strides, True)
...
class DualSkipConnection(nn.Module):
    ...
    def forward(self, x1, x2):
        x = torch.add(x1, x2)
        y = self.subblock(x, x.clone().detach().requires_grad_(True))
