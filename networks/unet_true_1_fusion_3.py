...
class UNet_true_1_fusion_3(nn.Module):
    '''
    encoder concat
    '''
    def __init__(
        ...
        def _create_block(
            ...
            return CustomContainer(DualInputModule(down1, down2), DualSkipConnection(subblock, self.dimensions, 2*c, c), up)
...
class DualSkipConnection(nn.Module):
    def __init__(self, subblock: nn.Module, spatial_dims: int, in_channel: int, out_channel: int, dim: int = 1,
                 act: tuple | str = Act.PRELU, norm: tuple | str = Norm.INSTANCE, dropout: float = 0.0, bias: bool = True,
                 adn_ordering: str = "NDA"):
